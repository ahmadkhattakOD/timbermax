...
export function useEditQuotation(quotationId: number) {
  ...
  // True when the quotation's saved address is a one-time address not in the
    ...
    try {
      ...
      if (customerResponse?.customerData) {
        ...
        if (matchingIndex !== -1) {
          ...
          // The quotation carries a one-time custom address that isn't in the
  ...
  // Adds an item created inline (via the Create Item modal) to the quotation.
  // Uses the created row directly to avoid stale `items` state, and exposes it in
  // the search dropdown list as well.
  const addCreatedItemToQuotation = async (created: any) => {
