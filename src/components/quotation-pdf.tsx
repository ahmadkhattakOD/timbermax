...
const QuotationPDF: React.FC<QuotationPDFProps> = ({ quotation }) => {
  ...
  const subtotal =
    quotation.quotation_items?.reduce(
      (sum: number, item: QuotationItem) => sum + (item.total_price || 0),
      0
    ) || 0;

  const taxRate = quotation.tax_rate || 0.1;
        ...
        <View style={styles.footer}>
          <View style={styles.notes}>
            <Text
              style={[
                styles.label,
                { marginBottom: 5, fontWeight: "bold" as const },
              ]}
            >
              Notes:
            </Text>
