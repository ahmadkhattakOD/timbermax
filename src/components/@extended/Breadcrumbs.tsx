...
export default function Breadcrumbs({
  ...
  let extension =
    countOccurrences(customLocation, "/") > 1
      ? customLocation.substring(customLocation.lastIndexOf("/") + 1)
      : "";
    ...
    if (!custom && menu.children) {
      menu.children.filter((collapse: NavItemType) => {
        if (collapse.type && collapse.type === "collapse") {
          ...
          if (customLocation.includes(collapse.url ?? "")) {
            setMain(menu);
            setItem(collapse);
          }
        }
            ...
            {title && !titleBottom && (
              <Grid item>
                <Typography variant="h2" sx={{ fontWeight: 700 }}>
                  ...
                  {extension !== "" &&
                    ` / ${extension.charAt(0).toUpperCase()}${extension.substring(1)}`}
                </Typography>
              </Grid>
            )}
